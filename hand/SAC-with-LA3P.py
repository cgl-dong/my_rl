...
class SAC:
    ''' 处理离散动作的SAC算法 '''
    def __init__(self, state_dim, hidden_dim, action_dim, actor_lr, critic_lr, alpha_lr, target_entropy, tau, gamma, device,buffer_size = 10000,min_priority = 1):
        # 策略网络
        ...
        self.replay_buffer = rl_utils.ActorPrioritizedReplayBuffer(state_dim,action_dim,buffer_size,device)
    ...
    def actor_learn(self,transition_dict):
        pass

    def critic_learn(self,transition_dict,uniform=False):
        state_batch, action_batch, next_state_batch, reward_batch, mask_batch, ind, _ = transition_dict
        ...
        if uniform:
            ...
            qf_loss /= torch.max(TD_loss_1.abs(), TD_loss_2.abs()).clamp(min=self.min_priority).pow(
                self.per_alpha).mean().detach()
        else:
        ...
        priority = torch.max(TD_error_1, TD_error_2).clamp(min=self.min_priority).pow(
            self.per_alpha).cpu().data.numpy().flatten()
    ...
    actor 和critic分开训练
    '''
    def update(self, transition_dict,prioritized_fraction=0.5):
