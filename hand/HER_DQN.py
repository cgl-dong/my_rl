...
class DQNAgent:
    ...
    def __init__(self, state_size, num_actions):
        ...
        self.Q_network = DuelingMLP(state_size, num_actions).to(device)
        self.target_network = DuelingMLP(state_size, num_actions).to(device)
    ...
    def optimize_model(self):
        ...
        non_final_mask = ~torch.tensor(batch.done).to(self.device)
        non_final_next_states = torch.stack([s for done, s in zip(batch.done, batch.next_state) if not done]).to(self.device)
        ...
        with torch.no_grad():
            ...
            next_state_values[non_final_mask] = self.target_network(non_final_next_states)[
                range(number_of_non_final), argmax_actions].to(device)
        ...
        for cycle in range(num_cycles):
            ...
            for episode in range(num_episodes):
                ...
                for step in range(num_bits):
                    ...
                    state_ = torch.cat((state, goal)).to(device)
