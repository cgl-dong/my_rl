...
parser.add_argument('--max_steps_total', type=int, default=int(2e5), help='number of maximum steps to take.') # bout 4 million
...
def run(args):
    ...
    logger = Logger(logdir=args.logdir, run_name=f"{OptionCriticFeatures.__name__}-{args.env}-{args.exp}-{datetime.datetime.now().strftime('%Y-%m-%d-%H-%M')}")
    ...
    while steps < args.max_steps_total:
        ...
        if args.switch_goal and logger.n_eps == 100:
        ...
        if args.switch_goal and logger.n_eps > 200:
            torch.save({'model_params': option_critic.state_dict(),
                        'goal_state': env.goal},
...
return_list = moving_average(return_list, 9)
plt.plot(episodes_list, return_list)
plt.xlabel('Episodes')
plt.ylabel('Returns')
plt.title('OC on {}'.format(args.env))
plt.show()